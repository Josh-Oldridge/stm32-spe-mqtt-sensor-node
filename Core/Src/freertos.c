...
void NetworkTask(void *argument) {
    ...
    // **Only process DHCP packets until IP is assigned**
    while (myConn.netif.ip_addr.addr == 0) {
        ...
        while ((p = (struct pbuf*) readPQ(&pQ[0])) != NULL) {
            ...
            if (ethType == 0x0800) {  // IPv4 packet
                ...
                if (ipProtocol == 17) {  // UDP (used by DHCP)
                    netif_input(p, &myConn.netif);
                } else {
                    DEBUG_MESSAGE("[NETWORK] Ignoring non-DHCP IPv4 packet...\n");
                    pbuf_free(p);
                }
            } else {
                pbuf_free(p);
        ...
        // ✅ Print the assigned IP address when available
        if (myConn.netif.ip_addr.addr != 0 && myConn.netif.ip_addr.addr != IPADDR_ANY) {
    ...
    if (myConn.netif.ip_addr.addr != 0 && dhcp_supplied_address(&myConn.netif)) {
        ...
        // ✅ Ensure valid networking setup
        netif_set_up(&myConn.netif);
        netif_set_default(&myConn.netif);  // ✅ Explicitly mark this as the default network interface
        //dhcp_stop(&myConn.netif);  // ✅ Stop DHCP to prevent further conflicts
        etharp_gratuitous(&myConn.netif);  // ✅ Send ARP Announcement
    }
    ...
    while (1) {
    	uint32_t now = BSP_SysNow();
    	    ...
    	    if ((now - lastPollTime) >= pollIntervalMs) {
    	        lastPollTime = now;
    	        sys_check_timeouts();
    	        LwIP_ADIN1110LinkInput(&myConn.netif);  // Process all packets
    	        if (myConn.netif.ip_addr.addr != 0 && dhcp_supplied_address(&myConn.netif)) {
    	            process_udp_query();
    	        } else {
    	            DEBUG_MESSAGE("[NETWORK] Skipping UDP queries, IP not assigned yet.\r\n");
    	        }
    	    }
        ...
        if ((now - lastArpPrint) >= 5000) {
            lastArpPrint = now;
            print_lwip_arp_table();
        }
        ...
        if (current_link_status != link_was_up) {
            ...
            if (!link_was_up) {
                DEBUG_MESSAGE("[NETWORK] Link Down - Attempting DHCP Renewal...\n");

                // 🛑 First, try to renew the DHCP lease instead of restarting it
                osDelay(3000);


                if (!dhcp_supplied_address(&myConn.netif)) {
                    // If renewal fails, then release and restart DHCP
                    DEBUG_MESSAGE("[NETWORK] DHCP Renewal failed, restarting DHCP...\n");
